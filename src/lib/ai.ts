...
- 黒ブツ: ゴミ・異物の付着（塗料外部からの混入、ブース清掃不足）
- 白ブツ: 塗料中の異物（塗料の凝集物、フィルター不良）
...
バッチ歩留まり管理:
- 1回の塗装で20枚前後を一括塗装する
- 歩留まり = (バッチ枚数 - NG枚数) / バッチ枚数 × 100
- 現実的には50%前後が一般的、90%以上が最終目標
- 歩留まり向上の鍵: 環境管理、粘度管理（岩田NK-2カップ滴下秒）、ガン設定の安定化
- 粘度: 15秒以下=低粘度、15-25秒=中粘度、25-35秒=高粘度（岩田NK-2基準）

2液塗料の硬化剤管理:
- 硬化剤比率は塗料メーカー指定の配合比率を守ることが重要
- 硬化剤不足: 硬化不良、密着不良、耐薬品性低下
- 硬化剤過多: 硬化収縮による割れ、黄変、可使時間短縮

乾燥工程:
- 複数ステップの乾燥が可能（乾燥1→インターバル→乾燥2→...）
- インターバル: フラッシュタイム（溶剤飛ばし）や重ね塗りまでの待ち時間
- 乾燥温度・時間は歩留まりに大きく影響する重要な管理項目
- 強制乾燥の温度が高すぎるとピンホール・発泡リスク

回答は必ず日本語で行ってください。`;

// --- A. 写真不具合検出 ---
export async function analyzePhoto(photoUrl: string, model?: string): Promise<{
  defects: { type: string; severity: string; description: string }[];
  overall: string;
  score: number;
}> {
  const response = await getClient().chat.completions.create({
    model: model || DEFAULT_MODELS.photo,
    messages: [
...
export async function assessRisk(
  ...
  const response = await getClient().chat.completions.create({
    model: model || DEFAULT_MODELS.risk,
    messages: [
...
export async function recommendSettings(
  ...
  const response = await getClient().chat.completions.create({
    model: model || DEFAULT_MODELS.recommend,
    messages: [
...
export async function queryData(
  ...
  const response = await getClient().chat.completions.create({
    model: model || DEFAULT_MODELS.query,
    messages: [
...
// --- E. 噴霧動画分析 ---
export async function analyzeVideo(frameUrls: string[], model?: string): Promise<{
  ...
  const response = await getClient().chat.completions.create({
    model: model || DEFAULT_MODELS.video,
    messages: [
...
export async function generateMonthlyReport(
  ...
  const response = await getClient().chat.completions.create({
    model: model || DEFAULT_MODELS.report,
    messages: [
