...
const SCHEMA = `paint_logs: id,painted_at,ambient_temp,ambient_humidity,booth_temp,workpiece_temp,paint_temp,paint_type,paint_product,hardener_ratio,dilution_ratio,viscosity_seconds,paint_lot,air_pressure,throttle_turns,needle_turns,gun_type,gun_distance,coat_count,surface_prep,drying_method,drying_temp,drying_time,drying_steps(jsonb: [{method,temp,time,interval}]),film_thickness,fan_power,defects(jsonb: NG枚数map),batch_size(int),defect_count(int),comment`;
