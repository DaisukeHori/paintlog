...
export default function FavoritesBar({ recentLogs, onSelect }: FavoritesBarProps) {
  ...
  return (
    <div className="px-4 mb-3">
      <div className="text-xs text-stone-500 mb-1.5">直近の記録からコピー</div>
      <div className="flex gap-2 overflow-x-auto pb-1">
        {recentLogs.map((log) => (
          <button
            key={log.id}
            onClick={() => onSelect(log)}
            className="flex-shrink-0 bg-white border border-stone-200 rounded-xl px-3 py-2 text-left touch-manipulation active:border-blue-300 min-w-[160px]"
          >
